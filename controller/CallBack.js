...
const {
    ...
    keepLogsSystem, getLogsUser, getLogsUserInMonth
...
const uploadFileCallBack = async (req, res) => {
    ...
    const resolve = await QueueInsertDigitalForm(formValues, req.body.status, req.body.id);
...
const setTokenJwt = async (req, res) => {
    try {
        if (req.body.params) {
            const token = jwt.sign({ userToken: req.body.params }, secretKeyMain, { expiresIn: '2h' });
            res.send(token);
        } else {
...
const setKeysX5t = async (req, res) => {
    try {
        ...
        if (x_has_me === token) {
            ...
            const setToken = jwt.sign({ localToken: publicKey }, secretKeyMain, { expiresIn: '2h' });
            res.send(setToken);
        } else {
            return res.status(401).json({ message: 'Unauthorized: Invalid token' });
        }
...
const settingDigital = async (req, res) => {
    try {
        res.json(await _setting(req.body.array, req.body.code));
    } catch (e) {
...
const getsettings = async (req, res) => {
    try {
        res.json(await _getsettings(req.body.params));
    } catch (e) {
        console.error('Error Function getEmployeeData:', e);
    }
}
